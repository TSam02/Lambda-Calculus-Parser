...
--
-- >>> build $ 'y' `lam` term 'x'
-- WAS *** Exception: The expression `(\y._1)` is malformed:
-- WAS  Error: The expression contains a free variable `x`
-- NOW The expression `(\y._1)` is malformed:
-- NOW   Error: The expression contains a free variable `x`
term :: Char -> Builder
